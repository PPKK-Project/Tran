...
박현우
챗 기능
프론트에서 백으로 email 보내서 user 찾는거 오류 안나게 하기

마이페이지 여행 목록 보여줄때
밑에 따로 공유된 여행목록 >> TravelPermission 에 권한이있으면 공유된거니까 자신의 id로 트래블 펄미션을 list로 불러오면 travel_id만 빼오기
이러면 연계되서 채팅방도 공유된 채팅방 여행목록채팅방 이게 떠야함
콘솔로그 더러운거 지우기

박선영 - PDF > pdf 변환 라이브러리 > html
김민수 - 여행계획페이지 개선
김찬우 - 항공권 페이지

# 2025-11-20

남은것들 - 채팅, 마이페이지 수정, 여행계획 페이지 출발지 도착지 지정가능하게

다 완료 되면 테스트, CI/CD, 배포 준비, 발표준비
각종 자료 문서화
